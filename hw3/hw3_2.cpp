...
bool test_converg(Field &phi, const vector<int> &points, double &old_value)
{
	double new_value = 0;
...
void solver(Field &phi)
{
	...
	while(converged == false)
	{
		...
		//now we will do the border
		for (int y = 0; y <= phi.ny(); ++y)
		{
			phi(0,y) = (4 * phi(1, y) - phi(2, y) )/ 3.;
			phi(phi.nx(),y) = (4 * phi(phi.nx()-1, y) - phi(phi.nx() - 2, y) )/ 3.;
		}
		for (int x = 0; x <= phi.nx(); ++x)
		{
			phi(x, 0) = (4 * phi(x, 1) - phi(x, 2) )/ 3.;
			phi(x, phi.ny()) = (4 * phi(x, phi.ny()-1) - phi(x, phi.ny()-2) )/ 3.;
		}
...
int main()
{
	int n = 150;
  	Field phi(n,n);
  	phi = 0.0;
  	set_inner_bourder(phi);
  	solver(phi);
  	out_put_graph_data(phi);
  	cout<<"phi(3/10,6/10)= "<<phi(phi.nx()*.3,phi.nx()*.6)<<endl;
  	cout<<"I used a smaller grid 150x150 so this would run faster so the answer is only accurete to 3sf\n";
  	cout<<" using a 1000x1000 we get an answer of  − 0.229121\n";
